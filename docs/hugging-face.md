...
By default the repositories are `<you>/<model.name>-lora` and
`<you>/<model.name>-mlx`, from the `name` in `config.yaml`. Override per run:
...
**Ollama** cannot pull these repositories: it runs GGUF and these are MLX. On a
new machine, fetch the adapter, fuse with `--dequantize`, and import as
`dragon export` does.
